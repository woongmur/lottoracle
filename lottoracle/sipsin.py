...
saju.py 가 세운 여덟 글자는 그 자체로는 이름표일 뿐이다. 명리학은 그중 일간
...
def ten_god(day_stem: int, target: int) -> str:
    """일간에서 본 상대 천간의 십신.
    ...
    """
    rel = (STEM_ELEMENT[target] - STEM_ELEMENT[day_stem]) % 5
    same = STEM_YIN[target] == STEM_YIN[day_stem]
    return TEN_GODS[rel * 2 + (0 if same else 1)]


def ten_god_of_branch(day_stem: int, branch: int) -> str:
    """지지의 십신. 지지를 본기 천간으로 바꾼 뒤 천간과 똑같이 본다."""
    return ten_god(day_stem, BRANCH_MAIN_QI[branch])


PILLAR_LABELS = ("년", "월", "일", "시")


def ten_gods_table(pillars, day_stem: int) -> list[dict]:
    """네 기둥의 여덟 글자에 십신을 매긴다. 일간만 '나' 다.

    pillars 는 [년주, 월주, 일주, 시주]. 시주를 모르면 세 개만 넘어온다.
    """
    out = []
    for i, p in enumerate(pillars):
        out.append({
            ...
            "stem": "일간" if i == 2 else ten_god(day_stem, p.stem),
            "branch": ten_god_of_branch(day_stem, p.branch),
            "isSelf": i == 2,
        })
    return out


def ten_god_counts(pillars, day_stem: int) -> dict[str, int]:
    """십신이 몇 번씩 나왔나. 일간 자신은 '나' 라 세지 않는다."""
    out = {g: 0 for g in TEN_GODS}
    for i, p in enumerate(pillars):
        if i != 2:
            out[ten_god(day_stem, p.stem)] += 1
        out[ten_god_of_branch(day_stem, p.branch)] += 1
    return out


def group_counts(counts: dict[str, int]) -> dict[str, int]:
    """십신 열 가지를 비겁·식상·재성·관성·인성 다섯 갈래로 합친다."""
    out = {g: 0 for g in ("비겁", "식상", "재성", "관성", "인성")}
    for name, n in counts.items():
        out[GOD_GROUP[name]] += n
    return out


# ---- 강약 -------------------------------------------------------------

def strength(pillars, day_stem: int) -> dict:
    ...
    def add(god: str, weight: int) -> None:
        nonlocal support, other
        if god in SUPPORTING:
            support += weight
        else:
            other += weight

    for i, p in enumerate(pillars):
        if i != 2:                            # 일간은 '나' 라 세지 않는다
            add(ten_god(day_stem, p.stem), 1)
        # 두 몫으로 세는 건 월'지' 뿐이다. 계절을 가리키는 글자가 그것이라서다.
        add(ten_god_of_branch(day_stem, p.branch), 2 if i == 1 else 1)
    ...
    if ratio >= 0.55:
        ...
        label = "중화"
    return {"label": label, "support": support, "other": other,
            "ratio": round(ratio, 3)}
...
SCHOLAR = {0: 5, 1: 6, 2: 8, 3: 9, 4: 8, 5: 9, 6: 11, 7: 0, 8: 2, 9: 3}
...
def _triad_of(branch: int) -> dict[str, int]:
    for group, table in _TRIAD.items():
        if branch in group:
            return table
    raise ValueError(f"삼합국에 없는 지지: {branch}")


def sinsal(pillars, day_stem: int) -> list[dict]:
    ...
    labels = PILLAR_LABELS[:len(pillars)]
    found: dict[str, list[str]] = {}

    def hit(name: str, target: int) -> None:
        for lab, b in zip(labels, branches):
            if b == target:
                found.setdefault(name, [])
                if lab not in found[name]:
                    found[name].append(lab)

    for target in NOBLEMAN[day_stem]:
        hit("천을귀인", target)
    hit("문창귀인", SCHOLAR[day_stem])

    bases = [branches[0]]                     # 년지
    if len(branches) > 2:
        bases.append(branches[2])             # 일지
    for base in bases:
        for name, target in _triad_of(base).items():
            hit(name, target)

    return [{"name": n, "at": [f"{lab}지" for lab in found[n]]}
            for n in ("천을귀인", "문창귀인", "역마", "도화", "화개") if n in found]
...
ELEMENTS_FULL_TEXT = ("목·화·토·금·수가 하나도 빠지지 않았습니다. 한쪽으로 쏠리지 "
                      "않아 어떤 환경에 놓여도 무난하게 적응한다고 봅니다.")


def _elements_note(elements: dict[str, int]) -> str:
    """오행 분포 한 줄. 다 있으면 그 자체가 좋은 말이고, 없으면 어느 것이 없는지."""
    missing = [e for e in ELEMENTS if not elements.get(e)]
    if not missing:
        return ELEMENTS_FULL_TEXT
    most = max(ELEMENTS, key=lambda e: elements.get(e, 0))
    return (f"{'·'.join(missing)}가 없고 {most}이 가장 많습니다. 없는 기운은 "
            f"그 자리를 사람이나 환경에서 채우게 된다고 봅니다.")


def reading(pillars, day_stem: int, elements: dict[str, int]) -> dict:
    """팔자 한 벌을 읽어서 화면에 넘길 묶음으로 만든다.

    pillars 는 [년주, 월주, 일주, 시주]. 태어난 시를 모르면 세 기둥만 넘긴다 —
    모르는 값을 지어내지 않으려고 시주는 아예 빼고 센다.
    """
    counts = ten_god_counts(pillars, day_stem)
    groups = group_counts(counts)
    st = strength(pillars, day_stem)
    stem_char = STEMS[day_stem]
    image, nature = DAY_STEM_TEXT[stem_char]

    # 두 번 이상 나온 십신을 많은 순서로. 같으면 십신 차례대로 — 순서가 흔들리면
    # 같은 생일에 다른 화면이 나온다.
    order = {g: i for i, g in enumerate(TEN_GODS)}
    strong = sorted((g for g, n in counts.items() if n >= 2),
                    key=lambda g: (-counts[g], order[g]))

    return {
        "dayStem": {"char": stem_char, "image": image, "nature": nature,
                    "element": ELEMENTS[STEM_ELEMENT[day_stem]]},
        "table": ten_gods_table(pillars, day_stem),
        "counts": counts,
        "groups": groups,
        "strong": [{"name": g, "count": counts[g],
                    "word": TEN_GOD_TEXT[g][0], "text": TEN_GOD_TEXT[g][1]}
                   for g in strong],
        "manyGroups": [{"name": g, "count": groups[g], "text": GROUP_MANY_TEXT[g]}
                       for g in groups if groups[g] >= 3],
        "noneGroups": [{"name": g, "text": GROUP_NONE_TEXT[g]}
                       for g in groups if groups[g] == 0],
        "strength": {**st, "text": STRENGTH_TEXT[st["label"]]},
        "sinsal": [{**s, "word": SINSAL_TEXT[s["name"]][0],
                    "text": SINSAL_TEXT[s["name"]][1]}
                   for s in sinsal(pillars, day_stem)],
        "elementsNote": _elements_note(elements),
    }
