 ...
 *  - 꼬리: 게임 뒤에 발행 일련번호(용지의 TR 번호)가 숫자로 더 붙는다.
 *
...
// 한 게임을 그 자리에서 떼어 내는 자. y 플래그라 lastIndex 자리에만 걸린다.
// 매치에 실패하면 lastIndex 가 0 으로 돌아가므로, 어디까지 읽었는지는 따로 센다.
const GAME_AT = /([A-Za-z])(\d{12})/y;

/** QR 문자열을 {drawNo, lines, kinds, serial} 로. 형식이 어긋나면 예외. */
export function parse(text) {
  ...
  let pos = 0;
  for (;;) {
    GAME_AT.lastIndex = pos;
    const m = GAME_AT.exec(body);
    if (!m) break;
    games.push(m);
    pos = GAME_AT.lastIndex;
  }
  ...
  if (tail && !/^\d+$/.test(tail)) {
    ...
    throw new Error(`QR 값에 알 수 없는 문자가 섞여 있습니다: '${tail.slice(0, 20)}'`);
  }
  ...
  return { drawNo, lines, kinds, serial: tail };
}
